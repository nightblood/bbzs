...
import com.zlf.bbzs.bean.PooBean;
import com.zlf.bbzs.bean.PooBeanDao;
...
public class PooFragment extends BaseFragment{
...
//                .setRangDate(startDate,endDate)//起始终止年月日设定
        ...
        List<PooBean> list = ((MainApplication) Utils.getApp()).getDaoSession().getPooBeanDao().queryBuilder().limit(1).orderDesc(PooBeanDao.Properties.DataTime).list();
        if (!list.isEmpty()) {
    ...
    private void commit() {
        try {
            ...
            PooBean bean = new PooBean(mDataTime, mEtKind.getText().toString(), mEtMemo.getText().toString());
            ((MainApplication) Utils.getApp()).getDaoSession().getPooBeanDao().insert(bean);
